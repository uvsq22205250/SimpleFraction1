...
Dans cet exercice, vous créerez une classe `fr.uvsq.td1.Fraction` représentant un nombre rationnel et une classe `fr.uvsq.td1.Main` qui testera les méthodes de la classe `fr.uvsq.td1.Fraction` **avec des assertions** (cf. [Utilisation d'assertions](https://koor.fr/Java/Tutorial/java_assert.wp)).
...
1. Sur la forge, créez le dépôt (_repository_) `SimpleFraction`;
   ...
   > Sia on coche README.md ou .gitignore en choissisant le langage de programmation, une fois le depot est créé, il sera créé avec un initial commit. En revanche, si on on coche pas, gitHub nous invite a créé un fichier pour commencer le repository supposons que README.md et on aura un premier commit nommé Create README.md commited now.
...
1. Localement, configurez `git` avec votre nom (`user.name`) et votre email (`user.email`) (cf. [Personnalisation de Git](https://git-scm.com/book/fr/v2/Personnalisation-de-Git-Configuration-de-Git));
    ```bash
    $ git config --global user.name "uvsq22205250"
    $ git config --global user.email abdeldjalil.smahi@ens.uvsq.fr
...
1. Initialisez le dépôt `git` local pour le projet (cf. [Démarrer un dépôt Git](https://git-scm.com/book/fr/v2/Les-bases-de-Git-D%C3%A9marrer-un-d%C3%A9p%C3%B4t-Git));
    ```bash
      cd C:\Users\jalil\IdeaProjects\TD-CmplmntProgrammation
      git init 
      
    ```
1. Dans votre IDE, créez la classe `fr.uvsq.td1.Fraction` (vide pour le moment) et la classe `fr.uvsq.td1.Main` (avec un simple affichage) dans le projet (cf. [Méthode `main`](https://docs.oracle.com/javase/specs/jls/se19/html/jls-12.html#jls-12.1.4));
   ...
   Validez les changements (cf. [Enregistrer des modifications dans le dépôt](https://git-scm.com/book/fr/v2/Les-bases-de-Git-Enregistrer-des-modifications-dans-le-d%C3%A9p%C3%B4t));
    ...
    ```
1. Ajoutez la méthode `toString` à la classe `fr.uvsq.td1.Fraction` (cf. [`Object.toString`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Object.html#toString())) qui retournera la chaîne `"Je suis une fraction."` et modifiez la classe `fr.uvsq.td1.Main` en conséquence;
...
## Partie II (à faire durant le TD) : compléter la classe `fr.uvsq.td1.Fraction`
Dans cet partie, vous compléterez les classes `fr.uvsq.td1.Fraction` et `fr.uvsq.td1.Main`.
Un exemple d'interface pour une telle classe est donné par la classe http://commons.apache.org/proper/commons-math/javadocs/api-3.6.1/org/apache/commons/math3/fraction/fr.uvsq.td1.Fraction.html[`fr.uvsq.td1.Fraction`] de la bibliothèque http://commons.apache.org/math/[Apache Commons Math].

Vous respecterez les consignes ci-dessous :
* chaque méthode de `fr.uvsq.td1.Fraction` sera testée dans `fr.uvsq.td1.Main` **avec des assertions** (cf. [Utilisation d'assertions](https://koor.fr/Java/Tutorial/java_assert.wp));
...
1. Ajoutez la comparaison de fractions selon l'ordre naturel (cf. [`java.lang.Comparable`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Comparable.html)).
   ```Java
    ...
    ```
1. Faites hériter votre classe `fr.uvsq.td1.Fraction` de la classe [`java.lang.Number`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Number.html) et complétez les méthodes
   ```Java
    // Vérifiez avec le code ci-dessous
    Number aNumber = java.math.BigDecimal.ONE;
    Number anotherNumber = new fr.uvsq.td1.Fraction(0, 2);
...
1. Donnez la ou les commandes shell permettant de
    ...
    1. compter le nombre de ligne d'un fichier
        ...
        # Répondre ici
        ```
    1. afficher les lignes du fichier `fr.uvsq.td1.Main.java` contenant la chaîne `uneVariable`
        ```bash
        # Répondre ici
        ```
